...
public class KthSmallElement2 {
	
	int KthSmallElmenet2(int arr[],int l,int r,int k) {
		...
		if (k > 0 && k <= r - l + 1) {
			...
			if (pos - l > k - 1)//if index of partitioned element is more then k, then recur for left part,
				return KthSmallElmenet2(arr, l, pos - 1, k);
				//changed right=pos-1
			
			return KthSmallElmenet2(arr, pos + 1, r, k - pos + l - 1); 
